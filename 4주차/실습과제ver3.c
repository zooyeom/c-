...
void Sell_product(int release_product[100],int stock_product[100]) {
    ...
    while(1){
        ...
        for (int i = 0; i < 5; i++) {
            scanf("%d", &number);
            temp_release_product[i]=number;
            if (temp_release_product[i] > stock_product[i]) {
                    printf("판매 수량이 재고를 초과했습니다. 다시 입력해주세요.\n");
                    for (int j = 0; j < i; j++) {
                        release_product[j] -= temp_release_product[j];
                    }
            }
        break; //break는 가장 가까운 반복문을 탈출시켜준다>>>while문 탈출
        }
        
        for(int i=0;i<5;i++){
            release_product[i]+=temp_release_product[i];
        }
